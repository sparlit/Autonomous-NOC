...
class Coder(BaseAgent):
    async def write_code(self, task: str):
        ...
        # Persistence to staging for verification
        from nanoc.core.evolution import SelfEvolutionManager
        sem = SelfEvolutionManager("nanoc", "staging")

        # Heuristic to find filename from task description or LLM output
        # In a real system, the Architect/Planner would provide this.
        # Here we'll try to extract it or use a default.
        filename = "nanoc/generated_code.py"
        if "in file" in task.lower():
            try:
                filename = task.lower().split("in file")[1].strip().split()[0].strip()
            except Exception:
                pass

        try:
            sem.prepare_staging()
            sem.apply_change_to_staging(filename, code)
            await self.log(f"Code persisted to staging: {filename}")
        except Exception as e:
            await self.log(f"Failed to persist code to staging: {e}")
