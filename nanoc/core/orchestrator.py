...
class Orchestrator:
    ...
    async def process_task(self, task: Dict[str, Any]):
        ...
        if role in self.agents:
            ...
            try:
                # Explicit role-based dispatch
                if role == "Architect" and hasattr(agent, "design_solution"):
                    result = await agent.design_solution(task['description'])
                elif role == "Planner" and hasattr(agent, "create_todo_list"):
                    result = await agent.create_todo_list(task['description'])
                elif role == "Coder" and hasattr(agent, "write_code"):
                    result = await agent.write_code(task['description'])
                elif role == "Reviewer" and hasattr(agent, "review_work"):
