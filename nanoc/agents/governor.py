    ...
    async def run_governance_cycle(self):
        while True:
            ...
            if action == "ABORT_HUMAN_REVIEW":
                ...
                self.memory.publish_event("system/scale", {"action": "up", "reason": "High backlog"})
            elif action == "SCALE_DOWN":
                await self.log("Autonomous scaling: decreasing agent capacity due to resource pressure.")
                self.memory.publish_event("system/scale", {"action": "down", "reason": "Resource pressure"})
